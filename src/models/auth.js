...
export default {
	...
	subscriptions: {
		setup ({dispatch, state, history}) {
			history.listen(location => {
				...
				dispatch({type: 'uptState', payload: {path}})
			})
			dispatch({type: 'changeLeft'})
		}
	},
	effects: {
		...
		*getModules ({payload}, {call, put, select}) {
			console.log(111);
			...
			if (data) {
				...
				let path = yield select(state => {
					return state.auth.path
				})
				yield put({type: 'changeLeftMenu', payload: {path}})
			} else {
	...
	reducers: {
		...
		loginSuccess (state, action) {
			return {...state, ...action.payload, logining: false}
		},
		...
		changeLeftMenu (state, action) {
			...
			if (left && left.length > 0) {
				if (left[0].childs && left[0].childs.length > 0) {
					...
					let temp = _.filter(left[0].childs, child => {
						return child.url === path
					})
					let first = _.first(temp) || {}
